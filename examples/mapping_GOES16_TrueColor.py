...
- Red: 0.64 µm, channel 2, "Visible Red"
- Green: 0.86 µm, channel 3, "Veggie Near-IR"
- Blue: 0.47 µm, channel 1, "Visible Blue
...
#
#
# - **Red** 0.64 µm (Channel **2**)
#       - Name: Red Visible
# - **Green** 0.86 µm (Channel **3**)
#       - Name: Veggie Near-IR
# - **Blue** 0.47 µm (Channel **1**)
#       - Name: Blue Visible
#
# Some important details to know about...
#
# **Value Range**: The data units of channel 1, 2, and 3 are in reflectance and have a range of
# values between 0 and 1. RGB values must also be between 0 and 1.
# 
...
# satellite is observing more light in that channel. Clouds appear white 
# because they reflect lots of red, green, and blue light. Notice that the land
# reflects a lot of "green" in the veggie channel because this channel is 
...
######################################################################
# The addition of the three channels results in a color image. Combine the
# three channels with a stacked array and display the image with `imshow`.
...
# `metpy.io` package to obtain the projection information from the file.
# Then use `Cartopy` to plot the image on a map. The GOES data and image is on a
# [geostationary projection](https://proj4.org/operations/projections/geos.html?highlight=geostationary).
...
# black. There is information, however, from other channels we can use to
# see clouds at night. To view clouds in portions of the domain experiencing
# nighttime, we will overlay the clean infrared (IR) channel over the true
# color image.
#
...
# IR channel is *brightness temperature*, NOT reflectance. We need to
# normalize the values between 0 and 1 before we can use it in our RGB image.
# In this case, we normalize the values between 90 Kelvin and 313 Kelvin.
...
######################################################################
#
# To fill in the dark area on the true color image, we will set each RGB 
# channel to equal the maximum value between the visible channels and the IR
# channels. When this is done, where RGB values are black in the true color
# image RGB = (0,0,0), it will be replaced with a higher value of the
# `cleanIR RGB`.
#
# Note that if the clean IR has really bright, cold clouds in the daylight,
# they will replace the color values in the true color image making the clouds
# appear more white. Still, it makes a nice plot and lets you see clouds when
# it is night.
...
# Rayleigh correction to the data to fix the blue light scattering, but that
# can be intense. More simply, we can make the colors pop out by adjusting the 
# image contrast. Adjusting image contrast is easy to do in Photoshop, and also
...
# -----------------------------------------------------
#
# but data files are so large that plotting is very slow. 
