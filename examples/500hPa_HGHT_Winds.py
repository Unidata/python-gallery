...
hght_500 = gaussian_filter(ds['Geopotential_height_isobaric'].sel(isobaric=500).data[0], sigma=3.0)
uwnd_500 = gaussian_filter(ds['u-component_of_wind_isobaric'].sel(isobaric=500).data[0], sigma=3.0) * units('m/s')
vwnd_500 = gaussian_filter(ds['v-component_of_wind_isobaric'].sel(isobaric=500).data[0], sigma=3.0) * units('m/s')

# Use MetPy to calculate the wind speed for colorfill plot, change units to knots from m/s
sped_500 = mpcalc.get_wind_speed(uwnd_500, vwnd_500).to('kt')
